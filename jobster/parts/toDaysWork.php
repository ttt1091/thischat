...
<?php
  ...
  while ($jobster_users = $jobster_user_query->fetch(PDO::FETCH_ASSOC)) {
    ...
    if(isset($jobster_working['start_time'])){ $startTime = $jobster_working['start_time']; } else { $startTime = ''; }
    if(isset($jobster_working['end_time'])){ $endTime = $jobster_working['end_time']; } else { $endTime = ''; }
...
?>
    ...
    <?php if(!empty($startTime)){ if($startTime>=$timeCountCheck && $startTime<$timeCountCheck+1 ){ echo ' jobster-col-working'; } elseif($startTime<=$timeCountCheck && $endTime>$timeCountCheck){ echo ' jobster-col-working'; } } ?>
    ">
    </td>
<?php $timeCountCheck++; } ?>
  </tr>
